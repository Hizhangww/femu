...
//	fprintf(fp_workload,"%lf %d %ld %u %x\n",curr_time, 0, sector_nb, length, 0);
	...
	ret = _FTL_WRITE_2(ssd, sector_nb, length);  //hsj add
#ifdef FTL_IO_LATENCY
	end_ftl_w = get_usec();
	if(length >= 128)
...
int64_t _FTL_WRITE(struct ssdstate *ssd, int64_t sector_nb, unsigned int length)
{
	...
	if(sector_nb + length > SECTOR_NB){
		printf("ERROR[%s] Exceed Sector number: SECTOR_NB[%d] sector_nb[%d] length[%d]\n", __FUNCTION__, SECTOR_NB, sector_nb, length);
        return FAIL;
    }
	else{
     ...
     */
    blocking_to = 0;
	static int64_t total_write_page_nb=0;//xyh add
	while(remain > 0){
	...
	printf("[XYH] total_write_page_nb[%ld]\n", total_write_page_nb);//xyh add
    if (blocking_to > curtime) {
...
//hsj add
int64_t _FTL_WRITE_2(struct ssdstate *ssd, int64_t sector_nb, unsigned int length)
{
    struct ssdconf *sc = &(ssd->ssdparams);
    int64_t SECTOR_NB = sc->SECTOR_NB;
    int64_t SECTORS_PER_PAGE = sc->SECTORS_PER_PAGE;
    ...
    //empty_block_entry* curr_sequence_entry = ssd->curr_sequence_entry;  //顺序块 //hhh add
	//empty_block_entry* curr_srand_entry = ssd->curr_srand_entry;  //随机块 //hhh add
	...
	while(remain > 0){
		...
		//printf old_ppn、lpn，看是否一样 
		printf("old_ppn = %d，lpn = %d\n", old_ppn, lpn);
		
		
		if(old_ppn == -1) {  //ppn == -1，写入顺序块，并且更新lpn对应ppn
			printf("----------------------------------------\n");  //hsj test
			//GET_NEW_PAGE_2(ssd, VICTIM_OVERALL, EMPTY_TABLE_ENTRY_NB, &new_ppn, curr_sequence_entry, 1);//hhh add
			ret = GET_NEW_PAGE_2(ssd, VICTIM_OVERALL, EMPTY_TABLE_ENTRY_NB, &new_ppn,1);// hhh add
			//printf old_ppn、new_ppn 
			printf("old_ppn == -1，old_ppn = %d，new_ppn = %d\n", old_ppn, new_ppn);
			
//			UPDATE_OLD_PAGE_MAPPING(ssd, lpn);//hhh add
//			UPDATE_NEW_PAGE_MAPPING(ssd, lpn, new_ppn);//hhh add
		}
		else if(old_ppn != -1) {  //ppn != -1，写入随机块，并且更新lpn对应ppn  
			//GET_NEW_PAGE_2(ssd, VICTIM_OVERALL, EMPTY_TABLE_ENTRY_NB, &new_ppn, curr_srand_entry, 2);//hhh add
			ret = GET_NEW_PAGE_2(ssd, VICTIM_OVERALL, EMPTY_TABLE_ENTRY_NB, &new_ppn, 2);//hhh add
			//printf old_ppn、new_ppn 
			printf("old_ppn != -1，old_ppn = %d，new_ppn = %d\n", old_ppn, new_ppn);
...
void GET_LBA(struct ssdstate *ssd) {
	int i = 0;
	unsigned int Length = 0;
    int64_t LBA_buf;
	char str[31];
    int64_t ret = 1;
    int64_t ret_FTL_WRITE;
	
	int fd = open("/home/hsj/femu-master_bak/hw/block/ssd/data.log", O_RDONLY);  //打开log文件，只读方式打开 
	FILE * fd_log = fopen("/home/hsj/femu-master_bak/hw/block/ssd/output.txt", "w");  //日志文件 
    
	if(fd == -1) {
        perror("open data.log error");
        return -1;
    }
    	
	if(fd_log == NULL) {
        printf("打开output.txt文件失败！\n");
        return -1;
    }
    
	while(ret > 0) {  //一行一行读，读到结尾停  太多了，读一部分试试
    	
    	ret = Readline(fd, str, sizeof(str));
    	if(ret == -1) {
    		perror("read data.log error");
    		return -1;
		}
		
        if(str[0] == 'L') {
			Length = 0;
        	for(i =0 ; i < 8; i++) {  //读一个Length，将文本中的字符转为数字存放在Length
                //字符转数字
                if('0' <= str[i + 2] && str[i + 2] <= '9') {
                    Length = (Length << 4) + 0 + str[i + 2] - '0';
                } else if('A' <= str[i + 2] && str[i + 2] <= 'F') {
                    Length = (Length << 4) + 10 + str[i + 2] - 'A';
                } else if('a' <= str[i + 2] && str[i + 2] <= 'f') {
                    Length = (Length << 4) + 10 + str[i + 2] - 'a';
                }
            } 
		}
        else if(str[0] == 'W') {  //判断该行为写数据 
            //写入lba数组
			LBA_buf = 0;
            for(i = 0; i < 8; i++) {  //读一个LBA，将文本中的字符转为数字存放在LBA_buf
                //字符转数字
                if('0' <= str[i + 2] && str[i + 2] <= '9') {
                    LBA_buf = (LBA_buf << 4) + 0 + str[i + 2] - '0';
                } else if('A' <= str[i + 2] && str[i + 2] <= 'F') {
                    LBA_buf = (LBA_buf << 4) + 10 + str[i + 2] - 'A';
                }else if ('a' <= str[i + 2] && str[i + 2] <= 'f') {
                    LBA_buf = (LBA_buf << 4) + 10 + str[i + 2] - 'a';
                }
            }
			...
			ret_FTL_WRITE = _FTL_WRITE_2(ssd, LBA_buf, Length);  //冷热数据算法 
			
			fprintf(fd_log, "LBA:%08x Length:%08x\n", LBA_buf, Length);
	    }
    }
    close(fd);
    fclose(fd_log);
