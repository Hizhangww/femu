		...
		for(i=0;i<PLANES_PER_FLASH;i++){
			...
			for(j=0;j<FLASH_NB;j++){
				...
				while(k > 0){
					...
					curr_entry->data_type = -1;// xyh add
				...
				curr_root->data = NULL; //xyh add
				curr_root->node = NULL; //xyh add
				curr_root->meta = NULL; //xyh add
				curr_root += 1;
			}
		}
...
empty_block_entry* GET_EMPTY_BLOCK(struct ssdstate *ssd, int mode, int mapping_index)
{
    ...
    while(ssd->total_empty_block_nb != 0){
		//printf("[XYH][%d], empty_block_table_index[%d]\n", __LINE__, ssd->empty_block_table_index);
        if(mode == VICTIM_OVERALL){
            curr_root_entry = (empty_block_root*)empty_block_list + ssd->empty_block_table_index;

			//if(curr_root_entry->empty_block_nb == 0){ //xyh add
            if(curr_root_entry->empty_block_nb == 0 && data_type == 3){ //xyh add
				...
				/*
					...
					3 -----> other
				*/
				//printf("[XYH][%d]: current data_type[%d] curr_root_entry->empty_block_nb[%d]\n",__LINE__, data_type, curr_root_entry->empty_block_nb);
				if (data_type == 0) { //DATA
					if (curr_root_entry->data != NULL) {
						curr_empty_block = curr_root_entry->data;
					}
					else {
						...
						curr_root_entry->data = curr_empty_block;
				...
				else if (data_type == 1) { //NODE
					if (curr_root_entry->node != NULL) {
						curr_empty_block = curr_root_entry->node;
					}
					else {
						if(curr_root_entry->empty_block_nb == 0)
						{
						...
						curr_empty_block = curr_root_entry->head;
						curr_root_entry->node = curr_empty_block;
						if(curr_root_entry->empty_block_nb == 1){
                        	curr_root_entry->head = NULL;
                        	curr_root_entry->empty_block_nb = 0;
                    	}
				...
				else if (data_type == 2) { //META
					if (curr_root_entry-> meta != NULL) {
						curr_empty_block = curr_root_entry->meta;
					}
					else {
						if(curr_root_entry->empty_block_nb == 0)
						{
						...
						curr_empty_block = curr_root_entry->head;
						curr_root_entry->meta = curr_empty_block;
						if(curr_root_entry->empty_block_nb == 1){
                        	curr_root_entry->head = NULL;
                        	curr_root_entry->empty_block_nb = 0;
                    	}
				...
				if (curr_empty_block->data_type == -1) {
					curr_empty_block->data_type = data_type;
				}
				//curr_empty_block = curr_root_entry->head;
				//xyh add
                if(curr_empty_block->curr_phy_page_nb == PAGE_NB){
					//xyh add
					if (data_type == 0) { //DATA
						curr_root_entry->data = NULL;
					}
					else if (data_type == 1) { //NODE
						curr_root_entry->node = NULL;
					}
					else if (data_type == 2) { //META
						curr_root_entry->meta = NULL;
					}
					else {
...
int INSERT_VICTIM_BLOCK(struct ssdstate *ssd, empty_block_entry* full_block)
{
	...
	new_v_b_entry->data_type = full_block->data_type; //xyh add
...
//hsj add
empty_block_entry* GET_EMPTY_BLOCK_2(struct ssdstate *ssd, int mode, int mapping_index, int type)// hhh add
{
    struct ssdconf *sc = &(ssd->ssdparams);
    void *empty_block_list = ssd->empty_block_list;
    int64_t EMPTY_TABLE_ENTRY_NB = sc->EMPTY_TABLE_ENTRY_NB;
    ...
    if(ssd->total_empty_block_nb == 0){  //没有空块返回NULL，打印错误报告
        printf("ERROR[%s] There is no empty block\n", __FUNCTION__);
        return NULL;
    }
    ...
    empty_block_root* curr_root_entry;
    empty_block_entry* curr_empty_block;
    while(ssd->total_empty_block_nb != 0){  //存在空块

        if(mode == VICTIM_OVERALL){
			//根据当前记录的 index 获取相应分组的空闲块链表 root
            

//            if(curr_root_entry->empty_block_nb == 0){  //当前 root 链表无空闲块，index++
//                ssd->empty_block_table_index++;
//                if(ssd->empty_block_table_index == EMPTY_TABLE_ENTRY_NB){  //到末尾处，重新返回开头位置，重新遍历
//                    ssd->empty_block_table_index = 0;
//                }
//                continue;
//            }
//            else{
				if (type == 1) { //顺序块
					curr_root_entry = (empty_block_root*)empty_block_list + ssd->sequence_empty_block_table_index;
					if (curr_root_entry->sequence != NULL) {
						curr_empty_block = curr_root_entry->sequence;
					}
					else {
						if (curr_root_entry->empty_block_nb == 0)
						{
						...
						curr_empty_block = curr_root_entry->head;
						curr_root_entry->sequence = curr_empty_block;
						if (curr_root_entry->empty_block_nb == 1) {
							curr_root_entry->head = NULL;
							curr_root_entry->empty_block_nb = 0;
						}
					...
					curr_root_entry = (empty_block_root*)empty_block_list + ssd->srand_empty_block_table_index;
					if (curr_root_entry->srand != NULL) {
						curr_empty_block = curr_root_entry->srand;
					}
					else {
						if (curr_root_entry->empty_block_nb == 0)
						{
						...
						curr_empty_block = curr_root_entry->head;
						curr_root_entry->srand = curr_empty_block;
						if (curr_root_entry->empty_block_nb == 1) {
							curr_root_entry->head = NULL;
							curr_root_entry->empty_block_nb = 0;
						}
				...
				if (curr_empty_block->type == 0) {
					curr_empty_block->type = type;
				}

                if(curr_empty_block->curr_phy_page_nb == PAGE_NB){
                	/* Eject Empty Block from the list */
                    INSERT_VICTIM_BLOCK(ssd, curr_empty_block);
                    curr_empty_block = curr_root_entry->head;  //把 root 链表中的头指针指向的闪存块实体当做活跃块，用以分配空闲页
                    if (type == 1) { //
						curr_root_entry->sequence = NULL;
						ssd->sequence_empty_block_table_index++;
						if (ssd->sequence_empty_block_table_index == EMPTY_TABLE_ENTRY_NB) {
							ssd->empty_block_table_index = 0;
						}
					}
					else if (type == 2) { //
						curr_root_entry->srand = NULL;
						ssd->sequence_empty_block_table_index++;
						if (ssd->srand_empty_block_table_index == EMPTY_TABLE_ENTRY_NB) {
							ssd->empty_block_table_index = 0;
						}
					}
                    /* Update Empty Block List */
...
//                    ssd->empty_block_table_index = 0;
//                }
                return curr_empty_block;  //活跃块中有空闲页，则将当前活跃块返回给 GET_NEW_PAGE() 函数来获取空闲页
//            }
        }
        else if(mode == VICTIM_INCHIP){
            curr_root_entry = (empty_block_root*)empty_block_list + mapping_index;
